...
export function FactorChart({ factors }: { factors: RiskFactor[] }) {
  ...
  return <ChartContainer config={{ contribution: { label: 'Risk contribution (points)', color: 'var(--chart-3)' } }} className="h-[270px] w-full aspect-auto"><BarChart data={data} layout="vertical" margin={{ right: 20, left: 0 }} accessibilityLayer><CartesianGrid horizontal={false} strokeDasharray="3 3"/><XAxis type="number" axisLine={false} tickLine={false} fontSize={10}/><YAxis type="category" dataKey="name" width={143} axisLine={false} tickLine={false} fontSize={10}/><ReferenceLine x={0} stroke="var(--border)"/><ChartTooltip content={<ChartTooltipContent/>}/><Bar dataKey="contribution" radius={3} barSize={17}>{data.map(d => <Cell key={d.id} fill={d.contribution < 0 ? 'var(--chart-1)' : 'var(--chart-3)'}/>)}</Bar></BarChart></ChartContainer>
}
