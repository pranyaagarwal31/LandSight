...
export function ModelImportanceChart({ data, measured = false }: { data: ModelPerformance['featureImportance']; measured?: boolean }) {
  return <div>
    <ChartContainer config={{ importance: { label: measured ? 'Learned feature importance (%)' : 'Absolute contribution share (%)', color: 'var(--chart-1)' } }} className="w-full aspect-auto" style={{ height: Math.max(280, data.length * 30) }}>
      <BarChart data={data} layout="vertical" margin={{ left: 0, right: 18 }} accessibilityLayer>
        ...
        <YAxis type="category" dataKey="name" width={132} interval={0} axisLine={false} tickLine={false} fontSize={9} />
    ...
    <ul className="sr-only">{data.map(item => <li key={item.name}>{item.name}: {item.importance.toFixed(1)} percent {measured ? 'learned feature importance' : 'of absolute contributions'}.</li>)}</ul>
  </div>
}
