...
export function AlertsPage() {
  ...
  async function update(id: string, status: Alert['status']) {
    ...
    if (!a || a.status === 'Resolved' || a.status === status || (status === 'Resolved' && !permissions.resolveAlerts)) return
    await mutate(current => current?.map(a => a.id === id ? { ...a, status } : a), { revalidate: false })
    addAudit(`${status} risk alert`, a.projectId, 'Success', { module: 'Alerts', details: `${a.category} (${a.id}): ${a.status} → ${status}. Updated for this demo session only.` })
