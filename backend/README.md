# LandSight backend — Phase 3 SHAP explainability
...
Phase 3 connects only the existing Risk Analysis and project-detail explanation panels to real TreeSHAP. The dashboard, demo adapter, navigation, GIS, analytics, role preview, audit logs, training pipeline, serialized release, and root deployment configuration remain unchanged. The frontend still works without Python/ML through its explicitly labeled demo-rule fallback. No database, real-data integration, or authentication is added.
...
`components/landsight/shap-explanation.tsx` is used by the existing `RiskExplanation` component in both views. SWR keys include the exact project inputs and do not retain another project's explanation. The same-origin Next.js `POST /api/ml/predict` proxy forwards bounded requests to the operator-controlled **`backend/connection.json`** base URL (default `http://127.0.0.1:8000`). It rejects oversized/non-JSON bodies, does not accept user-supplied upstream URLs, disables redirects/caching, and enforces a timeout. This route is read-only and persists nothing.

For local/preview operation, run the FastAPI service above alongside Next.js. **Publishing the Next.js app alone does not deploy Python.** To use SHAP on a hosted frontend, run this trusted backend separately and set `backend/connection.json` to its HTTPS origin reachable by the Next.js server; no client-exposed environment variable or new credential is needed for this prototype. Keep the upstream restricted to trusted server access and non-sensitive synthetic inputs until authentication/rate limiting is introduced. Without a reachable backend the application shows an explicit retryable, non-SHAP demo fallback. ML scores shown inside the explanation panel are deliberately distinguished from the unchanged demo scores, stages, recommendations and simulations elsewhere.
