...
import { fetchProjectDataset } from './project-data'
...
export const landSightService: LandSightService = {
  getProjects: async () => (await fetchProjectDataset()).projects,
  getProject: async id => (await fetchProjectDataset()).projects.find(p => p.id === id),
  ...
  getGISProjects: async () => (await fetchProjectDataset('/api/gis/projects')).projects,
